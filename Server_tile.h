...
inline void sendTileUpdate(int x, int y, int tile, int causedBy, ENetPeer* peer)
{
	...
	case 32: /*wrench*/
	{
		if (getItemDef(world->items[x + (y * world->width)].foreground).blockType == BlockTypes::LOCK)
		{
			...
			if (world->items[x + (y * world->width)].foreground == 2398)
			{
				...
				{
					Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wRare Item Store`|left|2398|\nadd_spacer|small|\nadd_textbox|`wYou can buy Rare Item Here|\nadd_spacer|small|\nadd_button|buyrl|`oBuy Royal Lock|\nadd_spacer|small|\nadd_button|buyel|`oBuy Emerald Lock|\nadd_spacer|small|\nadd_button|buyticket|`oBuy Summer Clash Ticket|\nadd_spacer|small||\nadd_button|buydav|`oBuy Da Vinci Wing|\nadd_spacer|small||\nadd_button|buygangel|`oBuy Golden Angel Wing|\nadd_spacer|small|\nend_dialog|gazettes|Cancel|");
				}
				return;
			}
			// VIP ENTRANCE DIALOG 
	...
	case 10456:
	{
		...
		if (x == static_cast<PlayerInfo*>(peer->data)->x / 32 && y == static_cast<PlayerInfo*>(peer->data)->y / 32)
		{
			if (static_cast<PlayerInfo*>(peer->data)->cloth_back != 10456) {
					...
					++currentPeer)
				{
					if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
						continue;
					sendSound(currentPeer, "transform.wav");
				}
			}
			else {
				sendClothes(peer);
	...
	case 7480:
	{
		...
		if (x == static_cast<PlayerInfo*>(peer->data)->x / 32 && y == static_cast<PlayerInfo*>(peer->data)->y / 32)
		{
			if (static_cast<PlayerInfo*>(peer->data)->cloth_back != 7480) {
