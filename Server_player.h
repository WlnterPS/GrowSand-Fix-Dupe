...
inline void banLoginDevice(ENetPeer* peer, const long long banDurationDefault, string sid, string mac)
{
	...
	const auto text = "action|log\nmsg|`4Sorry, this device or location is still banned for `w" + OutputBanTime(calcBanDuration(banDurationDefault)) + " Need help? Contact Kazura#0001!";
	const string dc = "https://discord.gg/fj6F3U6qTA";
	const auto url = "action|set_url\nurl|" + dc + "\nlabel|Join GrowSand Discord\n";
...
void PlayerCharm(ENetPeer* peer, int colum1, int colum2, int colum3, int colum4, int colum5, int colum6, int berapapacket)
{
...
inline void checkTimeBan(ENetPeer* peer, string username)
{
	...
	if (exist)
	{
		...
		if (fd.fail())
		{
			...
			Player::OnConsoleMessage(peer, "`5[Stream-ERROR]: `@ failed to check whether '" + username + "' is time banned. `5Please send this screenshot to GrowSand developers.");
		...
		if (bantimeleft < 1) return;
		auto text = "action|log\nmsg|`4Sorry, this account is temporary banned by `w" + bannedBy + " `4for `w" + OutputBanTime(calcBanDuration(banDuration)) + " `4for `w" + bannedReason + ". `4Need help? `8Contact Kazura#0001!";
	...
	if (exist)
	{
		...
		if (experimental::filesystem::exists("autoban/ip/" + ip + ".txt"))
		{
			...
			if (ifs.fail())
			{
				...
				Player::OnConsoleMessage(peer, "`5[Stream-ERROR]: `@ failed to check whether '" + static_cast<PlayerInfo*>(peer->data)->rawName + "' is auto banned by system. `5Please send this screenshot to GrowSand developers.");
				if (!static_cast<PlayerInfo*>(peer->data)->Console) enet_peer_disconnect_later(peer, 0);
			}
			else
			{
...
inline void checkIpBan(ENetPeer* peer)
{
	...
	if (exist)
	{
		string text = "action|log\nmsg|`4Sorry, this device or location is perma banned. Need help? Contact `2Kazura#0001!";
